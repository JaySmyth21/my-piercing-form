...
const WaitlistWatcher = ({ children }) => {
  ...
  useEffect(() => {
    const checkWaitlistStatus = async () => {
      try {
        // 🔹 1. Fetch location metadata (hours)
        ...
        if (
          todayHours?.isOpen &&
          Array.isArray(todayHours.periods) &&
          todayHours.periods.length > 0
        ) {
        ...
        // 🌅 3. Calculate tomorrow’s opening time
        ...
        if (
          tomorrowHours?.isOpen &&
          Array.isArray(tomorrowHours.periods) &&
          tomorrowHours.periods.length > 0
        ) {
          ...
          console.warn(`⚠️ Tomorrow’s waitlist hours missing or closed: ${tomorrowKey}`);
        }
    ...
    checkWaitlistStatus(); // ✅ Run immediately on mount
    const interval = setInterval(checkWaitlistStatus, 15000); // ⏱️ Re-run every 15 seconds
